...
/** Configure the API root (including version). Default `/api/v1`. */
...
export function resolveApiUrl(path: string) {
  ...
  if (path.startsWith("/api/v1")) return `${apiBaseUrl}${path.slice("/api/v1".length)}`;
...
export class QefroClient {
  ...
  exportCsv = async (slug: string, params: URLSearchParams) => {
    const token = localStorage.getItem(TOKEN_KEY);
    const res = await fetch(resolveApiUrl(`/api/v1/${slug}/export?${params}`), {
  ...
  printHtml = async (slug: string, id: string, format?: string) => {
    ...
    const res = await fetch(resolveApiUrl(`/api/v1/${slug}/${id}/print${q}`), { headers: tokenHeader() });
    if (!res.ok) {
  ...
  downloadPdf = async (slug: string, id: string, format?: string) => {
    const q = format ? `?format=${encodeURIComponent(format)}` : "";
    const res = await fetch(resolveApiUrl(`/api/v1/${slug}/${id}/print.pdf${q}`), { headers: tokenHeader() });
    if (!res.ok) {
      throw new ApiError(res.statusText, res.status);
    }
    const blob = await res.blob();
  ...
  upload = (file: File, kind: "file" | "image" = "file", onProgress?: (n: number) => void) =>
    new Promise<{ key: string; url: string; filename: string; content_type: string; size: number }>(
      (resolve, reject) => {
        const xhr = new XMLHttpRequest();
        xhr.open("POST", resolveApiUrl(`/api/v1/files?kind=${kind}`));
        const headers = tokenHeader();
        for (const [k, v] of Object.entries(headers)) xhr.setRequestHeader(k, v);
        xhr.upload.onprogress = (e) => {
          if (e.lengthComputable && onProgress) onProgress(e.loaded / e.total);
  ...
  downloadAttachment = (id: string, inline = false) =>
    fetch(resolveApiUrl(`/api/v1/attachments/${id}${inline ? "?disposition=inline" : ""}`), { headers: tokenHeader() }).then(
      async (res) => {
        if (!res.ok) {
          const data = await res.json().catch(() => ({}));
  ...
  importErrorsUrl = (id: string) => resolveApiUrl(`/api/v1/imports/${id}/errors`);
