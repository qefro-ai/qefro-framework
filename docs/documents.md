...
When the current status is in `lock_states`, PATCH is rejected. Changes go through operations (`submit`, `cancel`, `duplicate`, `amend`, or the app's own confirm/cancel handlers).
